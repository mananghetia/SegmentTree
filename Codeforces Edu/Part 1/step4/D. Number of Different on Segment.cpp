...
struct segTree
{
    ...
    void merge(segNode &cur,segNode &left,segNode &right)//change this function
    {
        ...
        for(int i=1;i<=40;i++)
        {
            cur.freq[i]=left.freq[i]+right.freq[i];
            cur.val+=(cur.freq[i]>0);
        }
    }
    ...
    void build(vector<int> &a,int l,int r,int id)
    {
        ...
        {
            seg[id].val=1;seg[id].freq[a[l]]=1;
            return;
        }
        int mid=(l+r)>>1LL,left=2*id+1,right=2*id+2;
        build(a,l,mid,left);
    ...
    void update(int ql,int pre,int val,int id=0)
    {
        // propogate(id);
        int l=seg[id].l;
        int r=seg[id].r;
        ...
        int mid=(l+r)>>1LL,left=2*id+1,right=2*id+2;
        if(ql<mid)
            update(ql,pre,val,left);
        else 
            update(ql,pre,val,right);
        merge(seg[id],seg[left],seg[right]);
    }
